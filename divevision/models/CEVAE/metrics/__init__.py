...
def _non_reference_metrics(img_rgb: np.ndarray, dict_key_suffix: str = "") -> Dict:
    ...
    return {
        f"niqe{dict_key_suffix}": niqe(img_rgb),
...
def compute(
    ...
    if inception is not None:
        ...
        rec_img_rescaled_IS = (
            torch.from_numpy(rec_img_rescaled_IS.transpose((0, 3, 1, 2)))
            .type(torch.uint8)
            .to(inception._device)
        )
    ...
    if gt_img_rgb is not None:
        ...
        if lpips is not None:
            ...
            metrics["lpips"] = (
                lpips(
                    torch.from_numpy(gt_img_rgb.transpose(2, 0, 1)).to(d) / 127.5 - 1.0,
                    torch.from_numpy(rec_img_rgb.transpose(2, 0, 1)).to(d) / 127.5
                    - 1.0,
                )
                .mean()
                .to("cpu")
        ...
        metrics["ssim"] = structural_similarity(
            gt_img_rgb, rec_img_rgb, data_range=255.0, win_size=11, channel_axis=2
        )
