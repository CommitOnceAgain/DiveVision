...
class DigitCaps(nn.Module):
    def __init__(
        ...
        self.W = nn.Parameter(
            torch.randn(1, np.prod(self.num_routes), 1, logits_num, num_capsules)
        )
        self.out_conv = nn.ConvTranspose2d(
            num_routes[0], channels, kernel_size=2, stride=2, padding=1
        )
    ...
    def forward(self, x):
        ...
        for iteration in range(num_iterations):
            ...
            if iteration < num_iterations - 1:
                a_ij = torch.matmul(
                    u_hat.transpose(3, 4), torch.cat([v_j] * num_routes, dim=1)
                )
                b_ij = b_ij + a_ij.squeeze(4).mean(dim=0, keepdim=True)
            else:
                u_j = c_ij * u_hat
        u_j = u_j.view(
            batch_size, self.num_routes[0], self.num_routes[1], self.num_routes[2], -1
        )
