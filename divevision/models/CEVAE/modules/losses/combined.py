...
def gdl_gradient_loss(
    gen_frames: torch.Tensor, gt_frames: torch.Tensor, alpha: int = 1
) -> torch.Tensor:
...
class ReconstructionLoss(nn.Module):
    ...
    def forward(
        ...
        to_01 = lambda img: torch.clip((img + 1) / 2, 0.0, 1.0)
        ssim_loss = 1 - ms_ssim(
            to_01(gt), to_01(reconstructions), data_range=1, size_average=True
        )  # return a scalar

        # reconstruction loss + perceptual loss + gdl_loss
        loss = (
            (self._pixel_weight * rec_loss)
...
class ReconstructionLossWithDiscriminator(nn.Module):
    def __init__(
        ...
        rank_zero_log_only(
            logger,
            f"ReconstructionLossWithDiscriminator running with {disc_loss} loss.",
        )
    ...
    def forward(
        ...
        # Reconstruction loss
        rec_loss, log_dict = self.reconstruction_loss(
            gt, reconstructions, optimizer_idx, global_step, last_layer, cond, split
        )
            ...
            if optimizer_idx == 1:
                ...
                if cond is None:
                    ...
                    logits_real = self.discriminator(
                        torch.cat((gt.detach(), cond), dim=1)
                    )
                    logits_fake = self.discriminator(
                        torch.cat((reconstructions.detach(), cond), dim=1)
                    )
