...
class CEVAE(BaseModel):
    def __init__(
        ...
        self.digitcaps = (
            DigitCaps(ddconfig["z_channels"]) if self._use_capsules else nn.Sequential()
        )
        self.decoder = Decoder(**ddconfig)
        self.quant_conv = (
            torch.nn.Conv2d(ddconfig["z_channels"], embed_dim, 1)
            if self._use_capsules
            else nn.Sequential()
        )
        self.post_quant_conv = (
            torch.nn.Conv2d(embed_dim, ddconfig["z_channels"], 1)
            if self._use_capsules
            else nn.Sequential()
        )
