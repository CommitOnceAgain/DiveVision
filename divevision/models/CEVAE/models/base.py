...
class BaseModel(pl.LightningModule):
    def __init__(
        ...
        self.loss = (
            instantiate_from_config(lossconfig) if lossconfig is not None else None
        )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        xrec = self(x)
        g_loss, log_dict = self.loss(
            y, xrec, 0, self.global_step, last_layer=self.get_last_layer(), split="val"
        )
        ...
        rec_metrics = {
            k: torch.tensor(v / bs, device=x.device) for k, v in rec_metrics.items()
        }
        self.log_dict(
            log_dict | rec_metrics,
            logger=True,
            on_step=True,
            on_epoch=True,
            batch_size=bs,
